...
export default function LoginNavbar() {
    ...
    function handleScroll() {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          const scrollThreshold = totalScrollable > 0 ? totalScrollable * 0.01 : 20; // 3% of page scroll or small fallback
  ...
  useEffect(() => {
    ...
    document.body.style.paddingTop = isSticky ? `${headerHeight}px` : "";
    return () => { document.body.style.paddingTop = ""; };
  }, [isSticky]);

  return (
    <header ref={navbarRef} className={`bg-black md:px-30 ${isSticky ? 'md:pr-30' : ''}`}>
      <div  className={` w-full transition-all duration-0 ${isSticky ? ' fixed top-0 md:pr-60  z-[100] shadow-2xl backdrop-blur-lg border-b border-white/10' : 'border-y border-white/10 bg-black'}`}>

        <div className="flex h-20 w-full items-center justify-between sm:px-6 lg:px-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="hidden md:block rounded-full bg-[#00C19C] px-9 py-4 text-sm font-semibold text-white shadow-[0_6px_20px_rgba(2,186,173,0.35)] transition-transform hover:scale-[1.02]"
            >
              Login
            </button>
